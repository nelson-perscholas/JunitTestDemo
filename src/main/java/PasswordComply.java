...
public class PasswordComply {
    ...
    private boolean verifyPasswordLength() {
        if(!passwordString.isEmpty()) {
            if(passwordString.length() >= minPasswordLength && passwordString.length() <=
                    maxPasswordLength) {
                return true;
            }
        }
    ...
    public boolean doesNotAlreadyExist() throws SQLException, NullPointerException {
        if(passwordString.equals("Abcd1234!")) {
    ...
    public boolean doesPasswordComply() {
        return verifyPasswordLength() && verifyAplhaNumeric() && hasAllowedSpecialCharacters() &&
